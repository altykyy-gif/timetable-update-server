...
const OWNER_PASSWORD = process.env.OWNER_PASSWORD || 'غيّر-كلمة-المرور-الافتراضية';
...
async function ensureData() {
  ...
  if (!existsSync(MANIFEST_PATH)) {
    await writeManifest('1.0.0', 'الإصدار الأساسي', 'نسخة البداية');
  }
}
...
async function writeManifest(version, notes, message = '') {
  const manifest = {
    version,
    htmlUrl: '/published/index.html',
    notes: notes || '',
    message,
    publishedAt: new Date().toISOString(),
    sha256: await sha256File(HTML_PATH)
  };
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && req.url === '/manifest.json') {
      return send(res, 200, await fs.readFile(MANIFEST_PATH, 'utf8'));
    }
    if (req.method === 'GET' && req.url === '/published/index.html') {
      return send(res, 200, await fs.readFile(HTML_PATH, 'utf8'), 'text/html; charset=utf-8');
    }
    if (req.method === 'GET' && req.url === '/health') return send(res, 200, { ok: true });
    if (req.method === 'POST' && req.url === '/api/publish') {
      const body = await readBody(req);
      if (body.password !== OWNER_PASSWORD) return send(res, 401, { error: 'كلمة مرور المالك غير صحيحة.' });
